...
-   In a model with one predictor, a null model would be that
    $\Prob{y_i=1}$ is not a function of $x_i$.

-   The difference in the deviance of the null model minus the deviance
    of the full model is
    $$\Delta_{D} = D_0 - D_1 = -2 \log  \frac{L(\hat{\alpha}\given\mathcal{D})}{L(\hat{\alpha},\hat{\beta}\given\mathcal{D})}.$$
...
-   In general, we can compare any two *nested* models using $\chi^2$
    test applied to differences in deviance.

-   The deviance of the subset model minus that of the full model will
    always be (approximately) distributed a $\chi^2$ with df equalling
